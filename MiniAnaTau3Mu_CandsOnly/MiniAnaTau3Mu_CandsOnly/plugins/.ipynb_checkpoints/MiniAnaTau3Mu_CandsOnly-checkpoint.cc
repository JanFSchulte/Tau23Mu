// -*- C++ -*-
// Package:    MiniAna2017/MiniAnaTau3Mu_CandsOnly
// Class:      MiniAnaTau3Mu_CandsOnly
//
/**\class MiniAnaTau3Mu_CandsOnly MiniAnaTau3Mu_CandsOnly.cc MiniAna2017/MiniAnaTau3Mu/plugins/MiniAnaTau3Mu_CandsOnly.cc
...
////
class MiniAnaTau3Mu_CandsOnly : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
    explicit MiniAnaTau3Mu_CandsOnly(const edm::ParameterSet&);
    ~MiniAnaTau3Mu_CandsOnly();
...
MiniAnaTau3Mu_CandsOnly::MiniAnaTau3Mu_CandsOnly(const edm::ParameterSet& iConfig){
    edm::InputTag algInputTag_;
...
MiniAnaTau3Mu_CandsOnly::~MiniAnaTau3Mu_CandsOnly(){
    // do anything here that needs to be done at desctruction time
    // (e.g. close files, deallocate resources etc.)
}
    
    
float MiniAnaTau3Mu_CandsOnly::dR(float eta1, float eta2, float phi1, float phi2){
...
float MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(pat::Muon m, vector<pat::TriggerObjectStandAlone> triggerObjects) {
...
typedef std::pair<const reco::MuonChamberMatch*, const reco::MuonSegmentMatch*> MatchPair;
const MatchPair& MiniAnaTau3Mu_CandsOnly::getBetterMatch(const MatchPair& match1, const MatchPair& match2) const{
...
float MiniAnaTau3Mu_CandsOnly::dX(const MatchPair& match) const{
...
float MiniAnaTau3Mu_CandsOnly::pullX(const MatchPair& match) const{
  if (match.first and match.second->hasPhi())
    return dX(match) /
...
float MiniAnaTau3Mu_CandsOnly::pullDxDz(const MatchPair& match) const{
  if (match.first and match.second->hasPhi())
...
float MiniAnaTau3Mu_CandsOnly::dY(const MatchPair& match) const{
...
float MiniAnaTau3Mu_CandsOnly::pullY(const MatchPair& match) const{
  if (match.first and match.second->hasZed())
    return dY(match) /
...
float MiniAnaTau3Mu_CandsOnly::pullDyDz(const MatchPair& match) const{
  if (match.first and match.second->hasZed())
...
void MiniAnaTau3Mu_CandsOnly::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup, const edm::Event& iEvent) {
  /*
...
void MiniAnaTau3Mu_CandsOnly::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    using namespace edm;
    using namespace std;
...
if(isAna){
    ...
    for(edm::View<reco::CompositeCandidate>::const_iterator TauIt=Cand3Mu->begin(); TauIt!=Cand3Mu->end(), trIn<Cand3Mu->size(); ++TauIt, ++trIn){
        ...
        if(!isMc){
            ...
            muonVector.push_back(&mu1);
            muonVector.push_back(&mu2);
            muonVector.push_back(&mu3);
        
            
            for (std::vector<const pat::Muon**>::iterator it = muonVector.begin(); it != muonVector.end(); ++it) {
                const pat::Muon* mu = **it;; // Dereference the iterator to get a pointer to the Muon object
        ...
        if(VtxIdV.size()>0 && vertices->size()>0) {
            ...
            if(transTracksAssoToVtx_copy.size()>1){
                ...
                if(PVertex.isValid() && TauIt->vertexChi2() >0 ){
                    ...
                    dR1_2017 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, TriggerObj_DsTau3Mu2017);
                    dR2_2017 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu2, TriggerObj_DsTau3Mu2017);
                    dR3_2017 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu3, TriggerObj_DsTau3Mu2017);
                    ...
                    dR1 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, TriggerObj_DsTau3Mu);
                    dR2 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu2, TriggerObj_DsTau3Mu);
                    dR3 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu3, TriggerObj_DsTau3Mu);
                    ...
                    if( isBParking){
                        dR1_Mu8 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, MuonsObjects_BPMu8);
                        dR2_Mu8 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu2, MuonsObjects_BPMu8);
                        dR3_Mu8 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu3, MuonsObjects_BPMu8);
                        Mu1_dRtriggerMatch_Mu8.push_back(dR1_Mu8);
                        Mu2_dRtriggerMatch_Mu8.push_back(dR2_Mu8);
                        Mu3_dRtriggerMatch_Mu8.push_back(dR3_Mu8);

                        dR1_Mu7 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, MuonsObjects_BPMu7);
                        dR2_Mu7 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu2, MuonsObjects_BPMu7);
                        dR3_Mu7 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu3, MuonsObjects_BPMu7);
                        Mu1_dRtriggerMatch_Mu7.push_back(dR1_Mu7);
                        Mu2_dRtriggerMatch_Mu7.push_back(dR2_Mu7);
                        Mu3_dRtriggerMatch_Mu7.push_back(dR3_Mu7);

                        dR1_Mu8_IP5 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, MuonsObjects_BPMu8_IP5);
                        Mu1_dRtriggerMatch_Mu8_IP5.push_back(dR1_Mu8_IP5);

                        dR1_Mu8_IP6 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, MuonsObjects_BPMu8_IP6);
                        Mu1_dRtriggerMatch_Mu8_IP6.push_back(dR1_Mu8_IP6);

                        dR1_Mu9_IP0 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, MuonsObjects_BPMu9_IP0);
                        Mu1_dRtriggerMatch_Mu9_IP0.push_back(dR1_Mu9_IP0);

                        dR1_Mu9_IP3 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, MuonsObjects_BPMu9_IP3);
                        Mu1_dRtriggerMatch_Mu9_IP3.push_back(dR1_Mu9_IP3);

                        dR1_Mu9_IP4 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, MuonsObjects_BPMu9_IP4);
                        Mu1_dRtriggerMatch_Mu9_IP4.push_back(dR1_Mu9_IP4);

                        dR1_Mu9_IP5 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, MuonsObjects_BPMu9_IP5);
                        Mu1_dRtriggerMatch_Mu9_IP5.push_back(dR1_Mu9_IP5);

                        dR1_Mu9_IP6 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, MuonsObjects_BPMu9_IP6);
                        Mu1_dRtriggerMatch_Mu9_IP6.push_back(dR1_Mu9_IP6);

                        dR1_Mu12_IP6 = MiniAnaTau3Mu_CandsOnly::dRtriggerMatch(*mu1, MuonsObjects_BPMu12_IP6);
...
for(edm::View<pat::Photon>::const_iterator pho=photons->begin(); pho!=photons->end(), j<photons->size(); ++pho, ++j){
    ...
    reco::BeamSpot beamSpot2;
    edm::Handle<reco::BeamSpot> beamSpotHandle2;
    iEvent.getByToken(token_BeamSpot, beamSpotHandle2);
    const reco::BeamSpot& beamspot2 = *beamSpotHandle2.product();
   if ( beamSpotHandle2.isValid() ) {
        beamSpot2 = *beamSpotHandle2;
        x_bs2 = beamSpot2.x0();
        y_bs2 = beamSpot2.y0();
        z_bs2 = beamSpot2.z0();
    } else {
...
void MiniAnaTau3Mu_CandsOnly::beginJob() {
    ...
    tree_->Branch("x_bs", &x_bs2);
    tree_->Branch("y_bs", &y_bs2);
    tree_->Branch("z_bs", &z_bs2);
...
void MiniAnaTau3Mu_CandsOnly::endJob() {
    std::cout << "Contents of MuonPt: ";
    for (const auto& elem : MuonPt) {
        std::cout << elem << " ";
    }
    std::cout << std::endl;
...
void MiniAnaTau3Mu_CandsOnly::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(MiniAnaTau3Mu_CandsOnly);

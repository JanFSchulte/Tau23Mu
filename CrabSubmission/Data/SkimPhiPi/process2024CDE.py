...
dataset_name = '/ParkingDoubleMuonLowMass{stream}/Run2024{era}-2024CDEReprocessing-{version}/MINIAOD'
template = '''
from CRABClient.UserUtilities import config, getUsername
config = config()

config.General.requestName = 'SkimPhiPi_2024ReRecoera{era}_{version}_stream{stream}_Mini_v1'
...
config.Data.outputDatasetTag = 'SkimPhiPi_2024ReRecoera{era}_{version}_stream{stream}_Mini_v1'
...
for portion in portions:
    ...
    for i in range(0,8):
        ...
        if a.resubmit:
           folder = "crab_projects/crab_SkimPhiPi_2024ReRecoera{era}_{version}_stream{stream}_Mini_v1".format(**args)
           print (folder)
